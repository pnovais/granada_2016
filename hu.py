...
def Humoments(obj,arquive,pca,re,cx,cy,tm,tm_std,mass_t,p):
    #pca = pop.copy()
...
#    idade media da populacao + desvio padrao
    ...
    tp_std = pca['age'].std()
#    densidade de massa
    massp_m = pca.mass.mean()
    massp_std = pca.mass.std()
    massp_t = pca.mass.sum()
    ...
    print('Densidade de massa total pop.: %5.3f Msun/pc^2' %massp_t)
    print('Densidade de massa media pop: %5.3f+-%5.3f Msun/pc^2' %(massp_m,massp_std))
    ...
    l=1
    '''
    massa media na populacao
    '''
    mass_p = pca.mass.sum()


    file.write('%s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s  %s %s %s %s %s %s %s %s %s\n' %(obj,p, Rm_re, SigR_re, tm,tm_std, tp, tp_std, mass_t,mass_p,
                  I1, I2, I3, I4, I5,I6, I7, a, b, f, tetha, exc, flong, SYM, Conc))
    return()
